...
public final class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        Usuario usuario = this.usuarioRepository.findById(login)
                .orElseGet(() -> this.usuarioRepository.save(new Usuario(login)));

        if (!usuario.getRoles().contains("ROLE_PRODUTO")) {
            usuario.addRole("ROLE_PRODUTO");
